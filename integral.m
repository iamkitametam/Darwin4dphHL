...
kin_e_x = zeros(Nt/Q,1); kin_e_y = zeros(Nt/Q,1); kin_e_z = zeros(Nt/Q,1);
kin_i_x = zeros(Nt/Q,1); kin_i_y = zeros(Nt/Q,1); kin_i_z = zeros(Nt/Q,1);

imp_e_x = zeros(Nt/Q,1); imp_e_y = zeros(Nt/Q,1); imp_e_z = zeros(Nt/Q,1);
imp_i_x = zeros(Nt/Q,1); imp_i_y = zeros(Nt/Q,1); imp_i_z = zeros(Nt/Q,1);

for i=1:Nt/Q
    kin_e_x(i) = me*sum(total_electrons_vx(i,:).^2); kin_e_y(i) = me*sum(total_electrons_vy(i,:).^2); kin_e_z(i) = me*sum(total_electrons_vz(i,:).^2);
    kin_i_x(i) = mi*sum(total_ions_vx(i,:).^2); kin_i_y(i) = mi*sum(total_ions_vy(i,:).^2); kin_i_z(i) = mi*sum(total_ions_vz(i,:).^2);
    
    imp_e_x(i) = me*sum(total_electrons_vx(i,:)); imp_e_y(i) = me*sum(total_electrons_vy(i,:)); imp_e_z(i) = me*sum(total_electrons_vz(i,:));
    imp_i_x(i) = mi*sum(total_ions_vx(i,:)); imp_i_y(i) = mi*sum(total_ions_vy(i,:)); imp_i_z(i) = mi*sum(total_ions_vz(i,:));
end
...
figure('Name','DYNAMICS INTEGRAL CHARACTERISTICS'); 
plotbrowser('on');

sp(1) = subplot(4,3,1);
plot(tau*Q:tau*Q:T,kin_e_x);
grid on; grid minor;
title('electron E^{kin}_x, [n_0mc^2]');

sp(2) = subplot(4,3,2);
plot(tau*Q:tau*Q:T,kin_e_y);
grid on; grid minor;
title('electron E^{kin}_y, [n_0mc^2]');

sp(3) = subplot(4,3,3);
plot(tau*Q:tau*Q:T,kin_e_z);
grid on; grid minor;
title('electron E^{kin}_z, [n_0mc^2]');

sp(4) = subplot(4,3,4);
plot(tau*Q:tau*Q:T,kin_i_x);
grid on; grid minor;
title('ion E^{kin}_x, [n_0mc^2]');

sp(5) = subplot(4,3,5);
plot(tau*Q:tau*Q:T,kin_i_y);
grid on; grid minor;
title('ion E^{kin}_y, [n_0mc^2]');

sp(6) = subplot(4,3,6);
plot(tau*Q:tau*Q:T,kin_i_z);
grid on; grid minor;
title('ion E^{kin}_z, [n_0mc^2]');

sp(7) = subplot(4,3,7);
plot(tau*Q:tau*Q:T,imp_e_x);
grid on; grid minor;
title('electron P_x, [n_0mc]');

sp(8) = subplot(4,3,8);
plot(tau*Q:tau*Q:T,imp_e_y);
grid on; grid minor;
title('electron P_y, [n_0mc]');

sp(9) = subplot(4,3,9);
plot(tau*Q:tau*Q:T,imp_e_z);
grid on; grid minor;
title('electron P_z, [n_0mc]');

sp(10) = subplot(4,3,10);
plot(tau*Q:tau*Q:T,imp_i_x);
grid on; grid minor;
title('ion P_x, [n_0mc]');

sp(11) = subplot(4,3,11);
plot(tau*Q:tau*Q:T,imp_i_y);
grid on; grid minor;
title('ion P_y, [n_0mc]');

sp(12) = subplot(4,3,12);
plot(tau*Q:tau*Q:T,imp_i_z);
grid on; grid minor;
title('ion P_z, [n_0mc]');
...
figure('Name','FIELD INTEGRAL CHARACTERISTICS'); 
plotbrowser('on');
...
figure('Name','TOTAL INTEGRAL CHARACTERISTICS'); 
plotbrowser('on');
...
sp2(1) = subplot(4,1,1);
plot(tau*Q:tau*Q:T, kin_total);
grid on; grid minor;
title('kinetic energy total, [n_0mc^2]');

sp2(2) = subplot(4,1,2);
plot(tau*Q:tau*Q:T, imp_total);
grid on; grid minor;
...
sp2(4) = subplot(4,1,4);
plot(tau*Q:tau*Q:T, kin_total + elmag_energy(Q:Q:Nt));
grid on; grid minor;
